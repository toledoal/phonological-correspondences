# Additive Structure of Phonological Correspondences — reproducible pipeline
...
.PHONY: help venv family algebra universes nulls additive repr-control superposition \
        cognate-eval distributions regimes chains patterns figures paper manual clean

help:                                        ## list targets
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | awk 'BEGIN{FS=":.*?## "}{printf "  \033[36m%-14s\033[0m %s\n",$$1,$$2}'

venv:                                        ## create venv and install deps (panphon, lingpy, networkx, …)
	python3.12 -m venv .venv && ./.venv/bin/pip install -q -r requirements.txt

# ---- analyses that read the bundled repertoire data/db/transf.db (no corpora needed) ----
