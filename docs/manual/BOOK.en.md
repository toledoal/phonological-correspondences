...
in $O$ (Chapter 10), and which of the $\sim$4000 generable IE operators are the ones actually avoided (Chapter
12). Confusing the two — treating the repertoire as if it were the subspace — was the central error the earliest
...
**The decomposition.** IE (7 branches): $|O_F|=97$, $\Sigma H_k=97$ ✓, $K_\cap=12$ (the atoms `{ant} {cont}
{voi}` are in every branch), $K_{1/2}=38$, exclusives $=26$. AN (11 branches): $|O_F|=48$, $K_\cap=4$, $K_{1/2}=18$,
...
from 42 to $\sim$15), so branch *richness* counts are sampling artifacts and must not be interpreted. The robust
...
| $\Delta(a,b)=\phi(a)\oplus\phi(b)$ | operator = symmetric feature-difference (the signature) |
...
Full operator tables, branch-by-branch repertoires, the exhaustive segmental graphs $G_S$ for both families, and
the per-chapter raw outputs live under `docs/` and `data/db/`, regenerated by the targets in Appendix B. Full-page
figures and the interactive graphs are collected here as the manual matures.
